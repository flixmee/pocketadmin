	...
	scenarios := []tests.ApiScenario{
		...
		},
		{
			...
			ExpectedContent: []string{
				...
				`"version":"1.1.0"`,
				`"collections":["workspaces","members","boards","custom_fields","task_groups","task_statuses","task_tags","tasks","task_custom_field_values","task_updates","task_attachments"]`,
				`"collectionCount":11`,
			},
		},
	}
...
func TestCollectionPresetPreview(t *testing.T) {
	...
	scenarios := []tests.ApiScenario{
		...
		{
			Name:   "resolved task-management preview",
			Method: http.MethodPost,
			URL:    "/api/collection-presets/task-management/preview",
			Body:   strings.NewReader(`{"prefix":"work"}`),
			Headers: map[string]string{
				"Authorization": collectionPresetSuperuserToken,
			},
			ExpectedStatus: http.StatusOK,
			ExpectedContent: []string{
				...
				`"name":"work_members"`,
				`"name":"work_boards"`,
				`"name":"work_custom_fields"`,
				`"name":"work_tasks"`,
				`"name":"work_task_custom_field_values"`,
				`"name":"work_task_attachments"`,
				`"targetCollection":"work_members"`,
				`"targetCollection":"work_task_statuses"`,
				`"targetCollection":"work_custom_fields"`,
