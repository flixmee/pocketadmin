...
func (m Collection) MarshalJSON() ([]byte, error) {
	...
	default:
		return json.Marshal(struct {
			baseCollection
			collectionBaseOptions
		}{m.baseCollection, m.collectionBaseOptions}, json.Deterministic(true))
	}
}
...
func onCollectionSave(e *CollectionEvent) error {
	...
	e.Collection.CollectionGroup = normalizeCollectionGroupName(e.Collection.CollectionGroup)
...
func onCollectionSaveExecute(e *CollectionEvent) error {
	...
	txErr := e.App.RunInTransaction(func(txApp App) error {
		...
		if err := e.App.EnsureCollectionGroup(e.Collection.CollectionGroup); err != nil {
			return err
		}
...
func (app *BaseApp) FindAllCollectionGroups() ([]string, error) {
	result := []string{}
...
func (app *BaseApp) RenameCollectionGroupWithIcon(oldName, newName string, icon *string) error {
	...
	if oldName == "" {
		return errors.New("missing collection group name")
	}
	if newName == "" {
	...
	return app.RunInTransaction(func(txApp App) error {
		...
		for _, collection := range collections {
			collection.CollectionGroup = newName
		...
		_, err = txApp.DB().
			Delete(collectionGroupsTable, dbx.HashExp{"name": oldName}).
			Execute()
...
func (app *BaseApp) DeleteCollectionGroup(name string) error {
	name = normalizeCollectionGroupName(name)
	if name == "" {
		return nil
	}
