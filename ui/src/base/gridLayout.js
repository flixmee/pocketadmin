...
export default class GridLayout {
    constructor(container, options = {}) {
        ...
        this.options = {
            ...DEFAULTS,
            ...options,
            breakpoints: { ...DEFAULTS.breakpoints, ...(options.breakpoints || {}) },
        };
    ...
    _onDragStart(e, item) {
        ...
        const onMove = ev => {
            ...
            const cellX = clamp(
                Math.round(localX / (targetGrid._colWidth + targetGrid._gap)),
                0,
                targetGrid._columns - previewW,
            );
    ...
    _onResizeStart(e, item) {
        ...
        const onMove = ev => {
            ...
            const targetW = clamp(
                Math.round((pxW + this._gap) / (this._colWidth + this._gap)),
                item.minW,
                Math.min(item.maxW, this._columns - item.x),
            );
